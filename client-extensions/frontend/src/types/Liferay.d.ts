declare namespace Liferay {
	...
	const on: (eventType: SpaLifecycleEventType, event: unknown) => void;
	const once: (eventType: SpaLifecycleEventType, event: unknown) => void;
	const fire: (name: string, payload?: any) => void;
	const Session: Session;
}

type SpaLifecycleEventType = 'beforeNavigate' | 'startNavigate' | 'endNavigate';
interface Detail {
	app: SpaApp;
	originalEvent: {
		type: SpaLifecycleEventType;
		target: SpaApp;
		preventDefault: () => void;
	};
	path: string;
...
interface Session {
	...
	set(key: string, value: Object | string, options?: any): Promise<any>;
}
...
interface Util {
	...
	navigate(url: string | URL, listeners?: Object): void;
	//refer: modules/apps/frontend-js/frontend-js-web/src/main/resources/META-INF/resources/liferay/modal/Modal.js
...
interface OpenToastProps {
	...
	message: string;
	type?: 'info' | 'warning' | 'secondary' | 'success' | 'danger';
	title?: string;
	symbol?: string;
	onClose?: () => void;
}
...
interface OAuth2Parameters {
	...
	homePageURL: string;
	_getOrRequestToken: () => Promise<TokenData>;
}

interface PortletKeys {
...
declare const CADDY_URL: string;
